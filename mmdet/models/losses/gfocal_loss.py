...
@weighted_loss
def decoupled_distribution_focal_loss(pred, label, weight_left, weight_right):
    r"""Distribution Focal Loss (DFL) is from `Generalized Focal Loss: Learning
    Qualified and Distributed Bounding Boxes for Dense Object Detection
    <https://arxiv.org/abs/2006.04388>`_.
...
@MODELS.register_module()
class DecoupledDistributionFocalLoss(nn.Module):
    r"""Distribution Focal Loss (DFL) is a variant of `Generalized Focal Loss:
    Learning Qualified and Distributed Bounding Boxes for Dense Object
    Detection <https://arxiv.org/abs/2006.04388>`_.
    ...
    def __init__(self, reduction='mean', loss_weight=1.0):
        super(DecoupledDistributionFocalLoss, self).__init__()
        self.reduction = reduction
        self.loss_weight = loss_weight
    ...
    def forward(self,
        ...
        reduction = (
            reduction_override if reduction_override else self.reduction)
        loss_cls = self.loss_weight * decoupled_distribution_focal_loss(
            pred, target, weight_left=weight_left, weight_right=weight_right, weight=weight, reduction=reduction, avg_factor=avg_factor)
        return loss_cls
