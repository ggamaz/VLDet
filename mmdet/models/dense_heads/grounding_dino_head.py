...
class DFINEHead(GroundingDINOHead):
    ...
    def _init_layers(self) -> None:
        ...
        self.pre_bbox_head = MLP(self.embed_dims, self.embed_dims, 4, 3)
        self.lqe_layers = nn.ModuleList(
            [copy.deepcopy(LQE(4, 64, 2, self.reg_max)) for _ in range(self.num_pred_layer)]
        )
    ...
    def forward(
        ...
        all_layers_outputs_refs = torch.stack(references)
        
        return all_layers_outputs_classes, all_layers_outputs_coords,all_layers_outputs_corners,all_layers_outputs_refs,pre_bboxes,pre_scores
    ...
    def loss(self, hidden_states: Tensor, references: List[Tensor],
        ...
        outputs = self(hidden_states, references, memory_text, text_token_mask)
