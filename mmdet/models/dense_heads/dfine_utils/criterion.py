...
import torchvision
from .dfine_utils import bbox2distance,box_iou,\
                    get_world_size,generalized_box_iou,is_dist_available_and_initialized
...
class DFINECriterion(nn.Module):
    ...
    def loss_labels_vfl(self, outputs, targets, indices, num_boxes, values=None):
        ...
        if values is None:
            ...
            ious, _ = box_iou(bbox_cxcywh_to_xyxy(src_boxes), bbox_cxcywh_to_xyxy(target_boxes))
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes, boxes_weight=None):
        ...
        loss_giou = 1 - torch.diag(
            generalized_box_iou(bbox_cxcywh_to_xyxy(src_boxes), bbox_cxcywh_to_xyxy(target_boxes))
        )
    ...
    def loss_local(self, outputs, targets, indices, num_boxes, T=5):
        ...
        if "pred_corners" in outputs:
            ...
            ious = torch.diag(
                box_iou(
                    bbox_cxcywh_to_xyxy(outputs["pred_boxes"][idx]), bbox_cxcywh_to_xyxy(target_boxes)
                )[0]
            )
    ...
    def get_loss_meta_info(self, loss, outputs, targets, indices):
        ...
        if self.boxes_weight_format == "iou":
            iou, _ = box_iou(
                bbox_cxcywh_to_xyxy(src_boxes.detach()), bbox_cxcywh_to_xyxy(target_boxes)
            )
            iou = torch.diag(iou)
